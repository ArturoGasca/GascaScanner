...
import gasca.com.bardcodescanner.Barcode.BarcodeGraphic
import gasca.com.bardcodescanner.Barcode.BarcodeGraphicTracker
import gasca.com.bardcodescanner.Barcode.BarcodeTrackerFactory
 ...
 */
class BarcodeCaptureActivity : AppCompatActivity(), BarcodeGraphicTracker.BarcodeUpdateListener, Interfaces.OptionsFragmentCallbacks {



    override fun onManualInput() {
        val layoutInflaterAndroid = LayoutInflater.from(this)
        val mView = layoutInflaterAndroid.inflate(R.layout.dialog_input, null)
        val alertDialogBuilderUserInput = AlertDialog.Builder(this)
        alertDialogBuilderUserInput.setView(mView)
        alertDialogBuilderUserInput
                .setCancelable(false)
                .setPositiveButton(getString(R.string.accept), { dialog, _ ->
                    val barcode = mView.txtBarcode.text.toString()
                    if (!TextUtils.isEmpty(barcode)){
                        dialog.dismiss()
                        nextActivity(barcode)
                    }
                })
    ...
    override fun onAutoFocusChanged(active: Boolean) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            mCameraSource?.focusMode = if (active) Camera.Parameters.FOCUS_MODE_CONTINUOUS_PICTURE else null
        }
    }
    ...
    private fun createCameraSource(autoFocus: Boolean, useFlash: Boolean) {
        ...
        val barcodeDetector = BarcodeDetector.Builder(context).setBarcodeFormats(Barcode.EAN_13).build()
    ...
    fun nextActivity(barcode: String){
        if(!starting){
