 ...
 */
package gasca.com.bardcodescanner.Barcode
...
class BarcodeGraphic internal constructor(overlay: GraphicOverlay<*>) : GraphicOverlay.Graphic(overlay) {
     ...
     */
    var detected: MutableMap<String, Int> = HashMap()
    internal fun updateItem(barcode: Barcode) {
        val qty = detected.getOrDefault(barcode.rawValue, 0) + 1
        detected[barcode.rawValue] = (qty)
        if(qty == 3){
